        ...
        Route::group([
            ...
            if (request()->is(config('crudbooster.ADMIN_PATH'))) {
                ...
                if ($menus) {
                    ...
                    CB::routeController('/', 'AdminController', static::$cb_namespace);
                }
            }


            CB::routeController('api_generator', 'ApiCustomController', static::$cb_namespace);
            ...
            foreach ($modules as $v) {
                if (@$v->path && @$v->controller) {
                    try {
                        CB::routeController($v->path, $v->controller, static::$cb_namespace);
                    } catch (\Exception $e) {
                        Log::error("Path = ".$v->path."\nController = ".$v->controller."\nError = ".$e->getMessage());
                    }
                }
